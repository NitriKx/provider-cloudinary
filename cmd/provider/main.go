...
import (
	...
	apisCluster "github.com/NitriKx/provider-cloudinary/apis/cluster"
	apisNamespaced "github.com/NitriKx/provider-cloudinary/apis/namespaced"
	"github.com/NitriKx/provider-cloudinary/config"
	"github.com/NitriKx/provider-cloudinary/internal/clients"
	controllerCluster "github.com/NitriKx/provider-cloudinary/internal/controller/cluster"
	controllerNamespaced "github.com/NitriKx/provider-cloudinary/internal/controller/namespaced"
	"github.com/NitriKx/provider-cloudinary/internal/features"
	"github.com/NitriKx/provider-cloudinary/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for Cloudinary").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-cloudinary"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-cloudinary",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped Cloudinary APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced Cloudinary APIs to scheme")
	...
	// The TF_CLI_CONFIG_FILE in the image points to the local mirror, so no
	// internet access is needed and the exact binary shipped in the image is used.
	...
	clusterOpts := tjcontroller.Options{
		...
		SetupFn:        clients.TerraformSetupBuilder(*terraformVersion, *providerSource, *providerVersion, nativeScheduler),
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudinary controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced Cloudinary controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped Cloudinary controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced Cloudinary controllers")
	}
