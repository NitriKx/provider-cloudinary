...
import (
	...
	clusterv1beta1 "github.com/NitriKx/provider-cloudinary/apis/cluster/v1beta1"
	namespacedv1beta1 "github.com/NitriKx/provider-cloudinary/apis/namespaced/v1beta1"
)
...
const (
	...
	errUnmarshalCredentials = "cannot unmarshal cloudinary credentials as JSON"
)
...
func TerraformSetupBuilder(version, providerSource, providerVersion string, scheduler terraform.ProviderScheduler) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		// Populate Terraform provider configuration from the credentials Secret.
		// Accepts either cloudinary_url alone, or the triple cloud_name + api_key + api_secret.
		ps.Configuration = map[string]any{}
		if v := creds["cloudinary_url"]; v != "" {
			ps.Configuration["cloudinary_url"] = v
		} else {
			if v := creds["cloud_name"]; v != "" {
				ps.Configuration["cloud_name"] = v
			}
			if v := creds["api_key"]; v != "" {
				ps.Configuration["api_key"] = v
			}
			if v := creds["api_secret"]; v != "" {
				ps.Configuration["api_secret"] = v
			}
		}
		return ps, nil
	}
