...
PROJECT_NAME ?= provider-cloudinary
PROJECT_REPO ?= github.com/NitriKx/$(PROJECT_NAME)
...
xpkg.build.provider-cloudinary: do.build.images
...
pull-docs:
	...
	@if [ -d "../terraform-provider-cloudinary/$(TERRAFORM_DOCS_PATH)" ]; then \
...
#   You can check the basic implementation here: https://github.com/crossplane/uptest/blob/main/internal/cloudinaryprovisionings/03-delete.yaml.tmpl.
